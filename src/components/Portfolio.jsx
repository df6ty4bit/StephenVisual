      ...
      if (item.images && item.images.length > 1) {
        intervalIds[`item-${item.id}`] = setInterval(() => {
          setCurrentImageIndex((prevIndexes) => {
            const nextIndex =
              (prevIndexes[`item-${item.id}`] || 0) + 1 >= item.images.length
                ? 0
          ...
          return (
            <div key={item.id} className="portfolio-item">
              <div
                ...
                onClick={() => {
                  if (isSingleImageItem && loadingStates[`item-${item.id}`])
                    return;
                  setZoomedImage(getCurrentImage(item));
                }}
              >
                {isSingleImageItem &&
                  loadingStates[`item-${item.id}`] === true && (
                    <div className="loading-spinner">
                      <FontAwesomeIcon icon={faSpinner} spin size="3x" />
                    </div>
                  )}
                {isSingleImageItem &&
                  loadingStates[`item-${item.id}`] === "error" && (
